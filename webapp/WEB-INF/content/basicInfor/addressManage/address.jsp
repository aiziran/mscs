...
<head>
	...
	function fncDetail(obj) {
		$('#tid').val(obj);		
		actSubmit($('#formPage'),'<c:url value="/basicInfor/addressManage/address!selectOne.action"/>');
	}
	//进入新增页面
	function fncAdd() {
		actSubmit($('#formPage'),'<c:url value="/basicInfor/addressManage/address!create.action"/>');
	}
	function fncDel() {
	 ...
	 if (!confirm("请确认是否提交？")) return;       
		actSubmit($('#formPage'),
				'<c:url value="address!deleteAll.action"/>');
	}
	//复选框全选
	...
	<SSTag:navigation/>
		<div id="LsearchArea">
			<form id="formSearch" name="formSearch" method="post" class="enterhandler" style="width: 1000px;"
				  action="<c:url value="/basicInfor/addressManage/address.action"/>" onEnter="this.submit()">
					<div>
					<table width="800">
					    <tr>
					     ...
					     <tr>
					        <th>手机号:</th><td ><input type="text" id="mobile" name="mobile" value="${data.mobile }" /></td>
							...
							<table class="LlistArea" id="listTab">
								...
								<s:iterator value="page.result" status="st">
									<tr nowrap="nowrap" onMouseOver="cfTrOver(this)" onMouseOut="cfTrOut(this)"
										...
										<td><a href="address!deleteId.action?tid=${id}">删除</a>
										</td>
									</tr>
								</s:iterator>
								<!-- 滚动条设置 -->
