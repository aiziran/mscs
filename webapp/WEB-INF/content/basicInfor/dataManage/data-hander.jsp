...
<script type="text/javascript" language="javascript">
	...
	function fncDetail(obj) {
		$('#id').val(obj);
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/dataManage/data!updateHanderPage.action"/>');
	}

	//进入新增页面
	function fncAdd() {
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/dataManage/data!insertHanderPage.action"/>');
	}
	...
	function fncDel() {
		...
		if (!confirm("请确认是否提交？"))
			return;
		actSubmit($('#formPage'), '<c:url value="integralshop!delete.action"/>');
	}
	//复选框全选
	...
	<!-- <SSTag:navigation />-->
		...
		<div id="LsearchArea">
			<form id="formSearch" name="formSearch" method="post"
				...
				action="<c:url value="/basicInfor/dataManage/data!hander.action"/>">
				<div>
					<table>
						<tr>
							<th nowrap="nowrap">机手:</th>
