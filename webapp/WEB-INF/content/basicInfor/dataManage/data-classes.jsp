...
<script type="text/javascript" language="javascript">
	...
	function fncDetail(obj) {
		$('#id').val(obj);
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/dataManage/data!updateClassesPage.action"/>');
	}
	...
	function fncAdd() {
		...
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/dataManage/data!insertClassesPage.action"/>');
	}
	...
	function fncDel() {
		...
		if (!confirm("请确认是否提交？"))
			return;
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/dataManage/data!deleteClasses.action"/>');
	}
	//复选框全选
...
<body onload="fncFixupListHead();">
	<div id="Lcontent">
		...
		<div id="LpageTitleArea">
			...
			<div>
				<a href="#">Home</a> &gt; <a href="#">基本信息管理</a> &gt; <a
					href="${ctx }/basicInfor/newsinfoManage/newsinfo.action?selectMenuId=1110"
					id="Lhere">班次信息</a>
			</div>
		</div>
		<!-- END sbox -->
		...
		<form id="formPage" method="post"
			action="<c:url value="/basicInfor/dataManage/data!classes.action"/>">
							...
							<table class="LlistArea" id="listTab">
								...
								<tr>
									<th><input type="checkbox" id="checkedALl"
										name="checkedALl" onclick="CheckedAll(this)" />
									</th>
									<th>序号</th>
									<th>班次</th>
								</tr>
								<s:iterator value="page.result" status="st">
									<tr>
										<td><input type="checkbox" id="checkBox" name="checkBox"
											value="${id}" onClick="CheckedSelf(this)" />
										</td>
