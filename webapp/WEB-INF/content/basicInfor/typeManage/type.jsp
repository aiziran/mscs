...
<script type="text/javascript" language="javascript">
 	//进入添加页面
	function fncAdd() {
		actSubmit($('#formPage'),'<c:url value="/basicInfor/typeManage/type!create.action"/>');
   }
	//进入修改页面
	function fncDetail(obj) {
		$('#tid').val(obj);
		actSubmit($('#formPage'),
				'<c:url value="type!selectOne.action"/>');
	}
	function fncDel() {
	 ...
	 if (!confirm("请确认是否提交？")) return;       
		actSubmit($('#formPage'),
				'<c:url value="type!deleteAll.action"/>');
	}
	//复选框全选
			...
			<c:if test="${!empty page.result}">
							...
							<table class="LlistArea" id="listTab">
								...
								<s:iterator value="page.result" status="st">
									<tr nowrap="nowrap" onMouseOver="cfTrOver(this)" onMouseOut="cfTrOut(this)">
										...
										<td onClick="fncDetail('${id}')">${archivename}</td>
										
										<td><a href="type!deleteId.action?tid=${id}">删除</a>
										</td>
									</tr>
								</s:iterator>
								<!-- 滚动条设置 -->
				...
				<ul class="LbtnArea" style="margin-right: 20px;">
					<li>
						...
						<input type="button" value="删除" onclick="fncDel()" id="checkBox" objType="bCreate"/>
					</li>
