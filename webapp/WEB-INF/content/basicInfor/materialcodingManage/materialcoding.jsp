...
<script type="text/javascript" charset="utf-8">
	...
	*/
	function chaXun() {
		postform.action="${ctx}/basicInfor/materialcodingManage/materialcoding.action";
		postform.submit();
	}
	...
	*/
	...
	function showDetail(id) {
		$.ajax({
			type:'post',//可选get
			url:'${ctx}/basicInfor/materialcodingManage/materialcoding!infoEcho.action',//这里是接收数据的程序
			...
			success:function(data){
				if(data!=null && data.length>0) {
					$.each(data, function(i, list){
						...
						$("#smttht").val(list.smttht);
						$("#remark").val(list.remark);
						$("#attrname").val(list.attrname);
						$("#units").val(list.units);
						$("#productinfo").val(list.productinfo);
						$("#arguments").val(list.arguments);
						$("#manufacturer").val(list.manufacturer);
						$("#componentclassify").val(list.componentclassify);
						$("#attrname2").val(list.attrname2);
						$("#arguments2").val(list.arguments2);
						$("#counts").val(list.counts);
	...
	*/
	function deleteInfo(id) {
		...
		window.location.href = "${ctx}/basicInfor/materialcodingManage/materialcoding!deleteInfo.action?id="+id;
	}
	/**
	录入/修改物料信息
	*/
	function saveAllInfo(status) {
		...
		if($("#counts").val()!=null && $("#counts").val().trim().length>0) {
			if(!regNum.test($("#counts").val())) {
				alert("[数量]请输入正确的数字或小数")
				return ;
			}
		}
	    var dataVal = {
	    		...
	    		attrname2: $("#attrname2").val(),
	    		arguments2: $("#arguments2").val()
	    };
	    $.ajax({
			type:'post',//可选get
			url:"${ctx}/basicInfor/materialcodingManage/materialcoding!isrepeat.action?status="+status,//这里是接收数据的程序
			...
			success:function(data){
				...
				if(data!=null && data.length>0) {
					if(data[0].flag===false) {
						if (!confirm("当前编码为["+data[0].codednum+"],是否"+statusVal+"?")) {
							return;
						}
						saveCpform.action = "${ctx}/basicInfor/materialcodingManage/materialcoding!savematerialinfo.action?status="+status;
					    saveCpform.submit();
					} else {
						if (!confirm("有相同属性物料,编码为["+data[1].codednum+"],当前生成的编码为["+data[0].codednum+"],是否继续"+statusVal+"?")) {
							return;
						}
						saveCpform.action = "${ctx}/basicInfor/materialcodingManage/materialcoding!savematerialinfo.action?status="+status;
					    saveCpform.submit();
	...
	<div id='tabs-886386' class='tabbable'
		...
		<ul class="nav nav-tabs" id="tabid">
			<li class="active"><a href="#panel-809020" data-toggle="tab"
				onclick="setTabIndex('2')">物料编码</a></li>
		</ul>
	</div>
	...
	<div class="panel panel-primary">
		...
		<div class="panel-body" style="padding: 0px;">
			<div class="table-responsive">
				<form id="formPage" action="${ctx}/basicInfor/materialcodingManage/materialcoding.action" method="post">
						...
						<tbody>
							<c:forEach items="${page.result}" var="info" varStatus="index">
								<tr>
									...
									<td onclick="showDetail(${info.id})" nowrap="nowrap">
									...
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.arguments2}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.productname}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.counts}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.productinfo}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.manufacturer}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.units}</td>
									<td onclick="showDetail(${info.id})" nowrap="nowrap">${info.remark}</td>
