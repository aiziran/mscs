...
<script type="text/javascript" language="javascript">
	...
	function fncSubmit() {
		   	actSubmit($('#formSearch'), '<c:url value="/basicInfor/userManage/user!inspectorDetail.action"/>');
  	}
</script>
</head>
<body class="body_sub">	
	<div id ="groupSupply" style="position:absolute; background-color:#ffffff;width:500px;display:block">
		...
		<form id="formPage" method="post" action="${ctx}/basicInfor/userManage/user!inspectorDetail.action">								
					<table class="LlistArea" >
						...
						<c:forEach items="${page.result}" var="com_user_m" varStatus="inx">
							...
							<tr onMouseOver="cfListOver(this)" onMouseOut="cfListOut(this)" 
								ondblclick="selectRadioAndClose('<c:out value="${com_user_m.userId}" />','<c:out value="${com_user_m.userName}" />');">
								
								<td>
									<input type="radio" id="chkRadio" name="chkRadio"  value="${com_user_m.userId}"/>
									</td>
								<td>
									<c:out value="${com_user_m.userId}" />
									
								</td>
								
								<td>
									
									<c:out value="${com_user_m.userName}" />
								</td>
