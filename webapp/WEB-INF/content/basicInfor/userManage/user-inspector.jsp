...
<script type="text/javascript" language="javascript">
	...
	function fncDetail(obj) {
		...
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/userManage/user!updateInspector.action"/>');
	}

	//进入新增页面
	function fncAdd() {
		actSubmit($('#formPage'),
				'<c:url value="/basicInfor/userManage/user!createInspector.action"/>');
	}
	...
	function showChildWindow1() {
		...
		var url = "<c:url value='/basicInfor/userManage/user!inspectorDetail.action'/>";
	...
	<div id="Lcontent">
		...
		<div id="LsearchArea">
			<form id="formSearch" name="formSearch" method="post"
				...
				action="<c:url value="/basicInfor/userManage/user!inspector.action"/>"
				onEnter="this.submit()">
				<div>
					<table>
						<tr>
							...
							<td><input type="text" id="queryUserId" name="queryUserId"
								...
								style="cursor: hand; margin-right: 10px" readonly />
							</td>

							<th nowrap="nowrap">用户名:</th>
							<td><input type="text" id="queryUserName"
								name="queryUserName" value="${data.queryUserName }" />
							</td>
