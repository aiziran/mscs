...
<script type="text/javascript" language="javascript">
	...
	function fncDel() {
	 ...
	 if (!confirm("请确认是否提交？")) return;       
		actSubmit($('#formPage'),
				'<c:url value="../emailManage/email!moveToSendSuc.action"/>');
	}
	//复选框全选
	...
	//清空搜索文本行的内容
	 ...
	 function selectEmailBySendto() {
			var name = document.getElementById("sendto").value;
			if(name!=null){
				actSubmit($('#formSearch'),
				'<c:url value="../emailManage/email!selectBySendto.action"/>');
			}
		...
		function orderByTime(){
			actSubmit($('#formPage'),
					'<c:url value="../emailManage/email!delSucOrderByTime.action"/>');
		}
		//按发送人排序
		function orderBySendto(){
			actSubmit($('#formPage'),
					'<c:url value="../emailManage/email!delSucOrderBySendto.action"/>');
		}
</script>
</head>
<body onload="fncFixupListHead();">
   ...
   <div id="navMenu" >
<ul>
 <li><a href="#" onclick="fncDel()">移到已发送</a>  </li>
    </ul>
    </div>
    </div>
...
<script type="text/javascript">cssdropdown.startchrome("navMenu")</script> 
		...
		<form id="formSearch" name="formSearch" method="post" class="enterhandler" style="width: 1000px;"
				  action="<c:url value="../emailManage/email!selectDelSucBySendto.action"/>" onEnter="this.submit()">
				<div>
					<table width="500">
						<tr>
							<th>按主题搜索电子邮件</th>
