...
<script type="text/javascript" language="javascript">
	...
	function fncSubmit() {
		   	actSubmit($('#formSearch'), '<c:url value="/common/popup/popup!salecontractList.action"/>');
  	}
</script>
</head>
<body class="body_sub">	
	<div id ="groupSupply" style="position:absolute; background-color:#ffffff;width:500px;display:block">
		...
		<!-- END sbox --> <!--AREA [View]-->
		<form id="formPage" method="post" action="${ctx}/common/popup/popup!salecontractList.action">								
					<table class="LlistArea" >
						...
						<c:forEach items="${page.result}" var="sale" varStatus="inx">
								<c:set var="index" value="${inx.index}" />
							<tr onMouseOver="cfListOver(this)" onMouseOut="cfListOut(this)" 
							ondblclick="selectRadioAndClose('<c:out value="${sale.id}" />','<c:out value="${sale.contractname}" />');">
								
								<td>
									<input type="radio" id="chkRadio" name="chkRadio"  value="${sale.id}"/></td>
								...
								<td>
									<c:out value="${sale.contractname}" />
								</td>
								<td>
									<c:out value="${sale.contractnum}" />
								</td>
							</tr>
						</c:forEach>
