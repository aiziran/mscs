...
<body class="body_sub">	
	<div id ="groupSupply" style="position:absolute; background-color:#ffffff;width:500px;display:block">
		...
		<!-- END sbox --> <!--AREA [View]-->
		<form id="formPage" method="post" action="${ctx}/common/popup/popup!selectDeputy.action">								
					<table class="LlistArea" >
						...
						<c:forEach items="${page.result}" var="deputy" varStatus="inx">
								<c:set var="index" value="${inx.index}" />
							<tr onMouseOver="cfListOver(this)" onMouseOut="cfListOut(this)" 
							ondblclick="selectRadioAndClose('<c:out value="${deputy.user_id}" />','<c:out value="${deputy.user_name}" />');">
								
								<td>
									<input type="radio" id="chkRadio" name="chkRadio"  value="${deputy.user_id}"/></td>
								<td>
									
									<c:out value="${deputy.user_id}" />
								</td>
								<td>
									<c:out value="${deputy.user_name}" />
								</td>
								<td>
									<c:out value="${deputy.dept_name}" />
								</td>
							</tr>
						</c:forEach>
