	...
	<div id ="groupSupply" style="position:absolute; background-color:#ffffff;width:500px;display:block">
		...
		<form id="formPage" method="post" action="${ctx}/common/popup/popup!myDept.action">								
					<table class="LlistArea" >
						...
						<c:forEach items="${page.result}" var="dept" varStatus="inx">
								<c:set var="index" value="${inx.index}" />
							<tr onMouseOver="cfListOver(this)" onMouseOut="cfListOut(this)" 
							ondblclick="selectRadioAndClose('<c:out value="${dept.deptid}" />','<c:out value="${dept.deptname}" />');">
								
								<td>
									<input type="radio" id="chkRadio" name="chkRadio"  value="${dept.deptid}"/></td>
