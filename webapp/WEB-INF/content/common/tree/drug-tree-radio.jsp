...
<script src="${ctx}/js/dtreeRadio.js"></script>
</head>
			  ...
			  <table border="0" cellspacing="0" cellpadding="0">
				<tr>
				  <td class="search_select_tree">
						<div id="treemenu" style="width:448px;height:200px;overflow-x:scroll;overflow-y:auto;display:">
						<SCRIPT type=text/javascript>						  
							var a = new dTree('a',jsBaseContent);
							a.config.useCookies=false;
							 <c:forEach items="${drugTypeTree}" var="item" varStatus="inx">				
							     <c:choose>
                 				 	<c:when test="${inx.index ==0 }">				
							    		a.add('<c:out value="${item.drugTypeCode}"/>',-1,'<c:out value="${item.drugTypeName}"/>',"javascript: void('S');");
							     	</c:when>
                 					<c:when test="${ inx.index > 0 }">
							    		a.add('<c:out value="${item.drugTypeCode}"/>','<c:out value="${item.pntTypeCode}"/>','<c:out value="${item.drugTypeName}"/>',"javascript: void('S');",'<c:out value="${item.drugTypeCode}"/>');
							   		</c:when>
               					</c:choose>
						    </c:forEach>						   
							document.write(a);	
