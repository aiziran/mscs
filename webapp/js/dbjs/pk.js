...
function newpos()
{
	...
	for(var i=1;i<cooks.length;i++){
		...
		ouli+="<li><img src='${ctx }/images/dbimages/closex.gif' align='absmiddle' onclick='delli("+wf[0]+")' /> "+wf[1].replace("&%#","/")+"</li>";
	}
	xul.innerHTML=ouli;
...
function dblist(id,event,linkproname)
{
    if(typeof(linkproname) != "undefined" && linkproname.length > 0)
    {
        ...
        if(cooks.length>2){alert("最多只允许添加2条");return;}else{SetCookie("listhc",cook+"/"+id+"_"+linkproname.replace("/","&%#"));zbyd(window.event||event);}
    ...
    else
    {
        ...
        for(var i = 0;i<nrid.length;i++)
        {
            if(nrid[i].id == "a"+id && nrid[i].checked)
            {
                ...
                else if(cooks.length>2)
                {
                    nrid[i].checked=false;alert("最多只允许添加2条");return;
                }
                else
                {
...
function comparepro()
{
	...
	if(url.length > 7)
	{
		 window.open("/car/front/fronttechparamManage/fronttechparam!seek.action");
	}
	else
	{
	    alert("请选择您要对比的车型");
