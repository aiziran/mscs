...
function dTree(objName,contextPath) {
	this.config = {
		...
		useRadio       : true,           //yanal添加，设置是否有添加单选按钮
		
		useCheckbox    : false           //yanal添加，设置是否使用复选框
	}
	var iconPath=contextPath+"/images/dTree_images";
	this.icon = {
...
dTree.prototype.setCheckedNodesSelected=function(titleValue){ 
	  for(var n=0; n<this.aNodes.length; n++){  
	      if(this.aNodes[n].title == titleValue){			      	 
		  var obj = document.getElementById("ra"+ n); 
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if(this.config.useRadio && node.pid != -1){
			str +='<input type="radio" class="radio_button" name="chk" id="r'+  this.obj + nodeId + '"  value="'+node.id+'"/>';
		}
...
dTree.prototype.co=function(id){   
    ...
    for(var n=0; n<this.aNodes.length; n++){   
        if(this.aNodes[n].id==id){   
            this.cbCollection[id]=document.getElementById("r"+this.obj+n);   
            break;   
        }   
    }   
...
dTree.prototype.getCheckedNodesByEndNode=function(){   
  ...
  for(var n=0; n<this.aNodes.length; n++){   
      ...
      if(cko!=null&&cko.checked==true){
    	 res=this.aNodes[n].title;
    	 break;
    	  }
  }   
  return res;   
