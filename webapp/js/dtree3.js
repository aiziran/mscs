...
function dTree(objName,contextPath) {   
    this.config = {   
        ...
        inOrder                 : false,   
        check:true     //added by wallimn 2009-02-05, to control whether checkbox is showed   
    };     
    var iconPath= contextPath+"/images/issm/tree/";//set the default value, be compatible with dtree   
    this.icon = {   
            root        : iconPath+'icon/root.gif',   
            folder      : iconPath+'folderClosed.gif',   
            folderOpen  : iconPath+'folderOpen.gif',   
            node        : iconPath+'icon/location.gif',   
            empty       : iconPath+'blank.gif',   
            line        : iconPath+'line1.gif',   
            join        : iconPath+'line3.gif',   
            joinBottom  : iconPath+'line2.gif',   
            plus        : iconPath+'plus3.gif',   
            plusBottom  : iconPath+'plus2.gif',   
            minus       : iconPath+'minus3.gif',   
            minusBottom : iconPath+'minus2.gif',   
            nlPlus      : iconPath+'plus.gif',   
            nlMinus     : iconPath+'minus.gif'  
        };   
...
//get the ids of the checkbox which check status is true;   
//added by wallimn, updated: 2009-02-13   
dTree.prototype.getCheckedNodesByEndNode=function(){   
  var res = new Array();   
  var cko;//checkobject   
  for(var n=0; n<this.aNodes.length; n++){   
      //alert("c"+this.obj+n+(document.getElementById("c"+this.obj+n).checked));   
      ...
      if(cko.checked==true){
    	  ...
    	  for(var b=0; b<this.aNodes.length; b++){
    		  if(this.aNodes[b].pid == this.aNodes[n].id){
    		      	res[res.length]=this.aNodes[n].title;
    			 i+=1;
    			 break;
    		  }
    	  }
    	 if(i==0) res[res.length]=this.aNodes[n].title;   
      }   
  }   
  return res;   
}   
dTree.prototype.getCheckedNodesByEndNodeName=function(){   
	  var res = new Array();   
	  var cko;//checkobject   
	  for(var n=0; n<this.aNodes.length; n++){   
	      //alert("c"+this.obj+n+(document.getElementById("c"+this.obj+n).checked));   
	      ...
	      if(cko.checked==true){
	    	  ...
	    	  for(var b=0; b<this.aNodes.length; b++){
	    		  if(this.aNodes[b].pid == this.aNodes[n].id){
	    		      	 res[res.length]=this.aNodes[n].name;
	    			 i+=1;
	    			 break;
	    		  }
	    	  }
	    	 if(i==0) res[res.length]=this.aNodes[n].name;   
	      }   
	  }   
	  return res;   
} 

dTree.prototype.setCheckedNodesSelected=function(titleValue){ 
	  var temp = titleValue.split(",");
	  for(var n=0; n<this.aNodes.length; n++){  
		  for(var j=0;j<temp.length;j++){
			  if(this.aNodes[n].title == temp[j]){			      	 
...
//added by wallimn   
dTree.prototype.cc=function(nodeId){   //只选当前的 
