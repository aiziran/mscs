...
public class Base64 {
	 ...
	 * @return 编码后的字符串
	 */
	public static String encodeStrUrlSafe(String str, String encoding) throws Exception {
		byte[] src = str.getBytes(encoding == null ? "UTF8" : encoding);
		String result = Base64.encodeBytes(src, Base64.URL_SAFE);
	 ...
	 * @return 解码后的字节数组
	 */
	public static String decodeUrlSafeStr(String src, String encoding) throws Exception {
		int i = src.length() % 4;
		if (i == 1)
		...
		byte[] bs = decode(src, Base64.URL_SAFE);

		return new String(bs, encoding == null ? "UTF8" : encoding);
	}
} // end class Base64
